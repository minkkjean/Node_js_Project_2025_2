...
gametestCREATE TABLE player_quests(
	player_id INT,
	quest_id INT,
...
-- 1. 새로운 퀘스트 추가
INSERT INTO quests (title, description, reward_exp, reward_item_id) VALUES
('중급자 퀘스트', '중급 난이도의 퀘스트를 완료하세요', 250, 2);
-- 2. 특정 플레이어의 모든 퀘스트 상태 조회
SELECT
    p.username AS '플레이어 이름',
    q.title AS '퀘스트 제목',
    pq.STATUS AS '진행 상태'
FROM players p
JOIN player_quests pq ON p.player_id = pq.player_id
JOIN quests q ON pq.quest_id = q.quest_id
WHERE p.player_id = 1;
-- 3. 가장 많은 경험치를 주는 퀘스트 출격
SELECT
    title AS '퀘스트 제목',
    reward_exp AS '경험치 보상'
FROM quests
ORDER BY
    reward_exp DESC LIMIT 1;