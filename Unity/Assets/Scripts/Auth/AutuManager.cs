...
public class AutuManager : MonoBehaviour
{
    ...
    public IEnumerator Login(string username, string password)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.PostWwwForm($"{SERVER_URL}/login", "POST"))
        {
            ...
            {
                var repsone = JsonConvert.DeserializeObject<LoginResponse>(www.downloadHandler.text);
                Debug.Log(repsone);
                SaveTokenToPrefs(repsone.accessToken, repsone.refreshToken, DateTime.UtcNow.AddMinutes(15));
